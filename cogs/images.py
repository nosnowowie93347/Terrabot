...
class Images(commands.Cog):
    ...
    async def fry(self, ctx: commands.Context, help="", amount="20"):
        ...
        if not successful:
            if amount == 0:  # Error command
                ...
                em.add_field(
                    name="Examples",
                    value="/fry https://imgur.com/a/wUChw7w | /deepfry (imageAttachment)",
                    inline=False,
                )
                em.color = 0x22BBFF
                await ctx.send(embed=em)
        ...
        img = np.array(
            img
        )  # convert PIL image to Numpy (CV2 works with numpy BRG arrays)
        # Convert RGB to BGR
        ...
        value = (
            int(amount) * 6
        )  # default multiplier, change depending on dark/light images
        ...
        kernel = np.array(
            [[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]]
        )  # how to modify a pixel based on surrounding pixels
        ...
        from skimage.transform import (
            swirl,
        )  # import works here but if at the beginning, breaks the code for some reason
        ...
        for x in range(8):
            img = swirl(
                ...
                radius=width / 1.4,
                center=(
                    random.randint(int(width / 4), int(width - width / 4)),
                    random.randint(int(height / 4), int(height - height / 4)),
                ),
            )
    ...
    async def radial(self, ctx: commands.Context, help="", amount="10"):
        ...
        if not successful:
            if amount == 0:  # Error command
                ...
                em.add_field(
                    name="Aliases", value="/blur /radialblur /funny", inline=False
                )
                em.add_field(
                    name="Examples",
                    value="/radialblur https://imgur.com/a/wUChw7w | /blur (imageAttachment) 30",
                    inline=False,
                )
                em.color = 0x22BBFF
                await ctx.send(embed=em)
    ...
    async def swirl(self, ctx: commands.Context, help="", amount="10"):
        ...
        if not successful:
            if amount == 0:  # Error command
                ...
                em.add_field(
                    name="Examples",
                    value="/swirl https://imgur.com/a/wUChw7w | /swirl (imageAttachment) 30",
                    inline=False,
                )
                em.color = 0x22BBFF
                await ctx.send(embed=em)
        ...
        from skimage.transform import (
            swirl,
        )  # import works here but not at the beginning
    ...
    async def warp(self, ctx: commands.Context, help="", amount="0"):
        ...
        if not successful:
            if amount == 0:  # Error command
                ...
                em.add_field(
                    name="Examples",
                    value="/warp https://imgur.com/a/wUChw7w | /warp (imageAttachment)",
                    inline=False,
                )
                em.color = 0x22BBFF
                await ctx.send(embed=em)
    ...
    async def getImage(
        self, ctx: commands.Context, command="", help="", amount="30", amount2="1"
    ):
        if help == "help":
        ...
        if help != "":  # if given url or number
            if self.validateURL(help):  # check if given url
                ...
                )  # use safari user agent in case they are blocking python UA
                ...
                if self.validateNum(amount, command):
                    ...
                    return (
                        "Invalid Number. Make sure the number is in a valid range. Type 'help' after the command to check the range",
                        0,
                        False,
                    )
            else:
                if self.validateNum(help, command):  # check if given number
                    ...
                    return (
                        "Invalid URL/Number. The 1st parameter you gave was invalid. Check if the link has an image or if the number is in a valid range",
                        0,
                        False,
                    )
        if help == "":
            try:  # get image from attachment
                ...
                for x in range(1, len(imgCache)):
                    try:
                        ...
                        continue
                return (
                    "No image found. Did you mean to show the help menu? Type 'help' after the command to show (without quotation marks)",
                    0,
                    False,
                )
