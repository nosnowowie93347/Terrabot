...
class Admin(commands.Cog):
    ...
    @commands.command(
        description="Pins the message with the specified ID to the channel"
    )
    @commands.guild_only()
    ...
    @commands.command(
        usage="<user> <reason>",
        description="A nice ban command to use on rulebreakers.",
        brief="When you're just not having it",
    )
    ...
    async def ban(self, ctx, member: discord.Member, *, reason: str):
        ...
        banembed = discord.Embed(
            title=f"{member} banned successfully",
            description=f"{member} banned for {reason}",
        )
        dmembed = discord.Embed(
            title="Banned",
            description="You were banned from {} for {}".format(ctx.guild, reason),
        )
    ...
    @commands.command(
        usage="<userid> <reason>",
        description="Ban ppl by id. Bot and user must have ban perms",
        aliases=["idban", "banbyid"],
    )
    ...
    async def hackban(self, ctx, memberid: discord.Object, *, reason: str):
        ...
        embed = discord.Embed(
            title=f"Successfully banned {memberid.id}",
            description=f"{memberid.id} banned for {reason}",
            color=0xFF00F6,
        )
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str):
        ...
        description = (
            f"You've been a bad boy/girl, {member.mention}. You've been kicked."
        )
        kickembed = discord.Embed(
            title="User kicked",
            description=description,
            color=0xF6D025,
            footer="Powered by: Terrabot",
        )
        pmembed = discord.Embed(
            title="You've been kicked.",
            description=f"You've been kicked from {ctx.guild} for {reason}",
        )
    ...
    async def addrole(self, ctx, member: discord.Member, *, role: discord.Role):
        ...
        if role in member.roles:
            return await ctx.send(
                "Cannot add this role to this user as they already have it! Nice try tho."
            )
        if role.position >= ctx.author.top_role.position:
            return await ctx.send(
                "Nice try. You can't add a role that's higher than your own. Get an admin to do it"
            )
        if role.position == ctx.me.top_role.position:
            return await ctx.send(
                Language.get("moderation.no_editrole_highest_role", ctx)
            )
        if role.position > ctx.me.top_role.position:
            return await ctx.send(
                Language.get("moderation.no_editrole_higher_role", ctx)
            )

        await user.add_roles(
            role,
            reason=Language.get("moderation.addrole_reason", ctx).format(
                role, ctx.author
            ),
        )
        await ctx.send(
            Language.get("moderation.addrole_success", ctx).format(name, user)
        )
    ...
    async def removerole(self, ctx, member: discord.Member, *, rolename: discord.Role):
        ...
        embed = discord.Embed(
            title="Role removed",
            description="Role {} removed from {}".format(rolename, member),
            color=0xF6D025,
            footer="Powered by Terrabot",
        )
        await ctx.send(embed=embed)
    ...
    async def massban(self, ctx, *, ids: str):
        ...
        if len(failed_ids) != 0:
            await ctx.send(
                Language.get("moderation.massban_failed_ids", ctx).format(
                    ", ".join(ids)
                )
            )
        await ctx.send(
            Language.get("moderation.massban_success", ctx).format(success, len(ids))
        )
        for channel in guild.channels:
            if channel.name == "logs":
                await channel.send(
                    Language.get("moderation.massban_success", ctx).format(
                        success, len(ids)
                    )
                )
    ...
    async def delrole(self, ctx, *, role: discord.Role):
        ...
        embed = discord.Embed(
            title="Role Deleted",
            description="Deleted the role {}".format(role.name),
            color=0xFF00AE,
        )
    ...
    @commands.command(
        usage="<name> <slomodetime>", description="create a text channel."
    )
    @commands.guild_only()
    ...
    async def edittc(self, ctx, newname):
        ...
        embed = discord.Embed(
            title="Renamed Channel Successfully",
            description=f"The channel has been renamed to {newname}",
        )
        await ctx.send(embed=embed)
    ...
    @commands.command(
        aliases=["cs"], description="Sends a nice fancy embed with some channel stats"
    )
