...
def create_tables():
    cur.execute(
        """CREATE TABLE IF NOT EXISTS guilds(id INTEGER, type TEXT, value TEXT)"""
    )
    cur.execute(
        """CREATE TABLE IF NOT EXISTS blacklist(id INTEGER, name TEXT, discrim TEXT, reason TEXT)"""
    )


def insert_data_entry(id, type, value):
    cur.execute(
        """INSERT INTO guilds(id, type, value) VALUES (?, ?, ?)""", (id, type, value)
    )
    conn.commit()
...
def update_data_entry(id, type, value):
    ...
    cur.execute(
        """UPDATE guilds SET value=(?) WHERE id=(?) AND type=(?)""", (value, id, type)
    )
    conn.commit()
...
def blacklistuser(id, name, discrim, reason):
    cur.execute(
        """INSERT INTO blacklist(id, name, discrim, reason) VALUES (?, ?, ?, ?)""",
        (id, name, discrim, reason),
    )
    conn.commit()
...
def getblacklist():
    ...
    rows = cur.fetchall()
    for row in rows:
        entry = 'ID: "{}" Name: "{}" Discrim: "{}" Reason: "{}"'.format(
            row["id"], row["name"], row["discrim"], row["reason"]
        )
